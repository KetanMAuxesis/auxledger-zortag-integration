...
open class LoginActivity : AppCompatActivity(), BitteBitte {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        login = verticalLayout {
                ...
                onClick {
                    if (name.getInput() == "a" && pass.getInput() == "a") {
                        ...
                        btnAuthenticate.onClick {
                            val url = "zorsecure://"
                            val app = "market://details?id=com.zortag.zorsecure"

                            val intent = Intent(Intent.ACTION_VIEW, Uri.parse(url))
                            when {
                                intent.resolveActivity(packageManager) != null -> startActivity(intent)
                                else -> startActivity(Intent(Intent.ACTION_VIEW, Uri.parse(app)))
                            }
                        }
            ...
            linearLayout {
                ...
                imageView(R.drawable.logo_zortag).lparams {
                    gravity = Gravity.CENTER
                    height = 130
                    width = 320
                }
